...
export function ActiveTimerList() {
  ...
  return (
    <section className="rounded-2xl border border-amber-200 bg-amber-50 p-4 shadow-sm dark:border-amber-700/40 dark:bg-amber-900/20">
      <h2 className="text-sm font-semibold uppercase tracking-wide text-amber-800 dark:text-amber-200">Active timers</h2>
      <div className="mt-3 grid gap-3 sm:grid-cols-2">
        {timers.map((timer) => (
          <TimerCard key={timer.id} timer={timer} now={now} onStop={stopTimer} onCancel={cancelTimer} />
        ))}
      </div>
    </section>
  );
